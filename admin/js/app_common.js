...
mm._isReview = mm.is.layout('review');
	...
	var base = {
		...
		intersectionHandler: function (__entry, __io, __isForce) {
			...
			if (__entry.isIntersecting || __isForce === true) {
				...
				if (target.option._isOnce) base.removeObserve(_.findIndex(base.targets, { $el: target.$el, callback: target.callback, io: target.io }));
			}
...
mm.scroll = (function () {
	...
	var base = {
		...
		toggle: function (__is) {
			...
			if (mm.class.some($html, ['__bom', '__modal'])) return;
	...
	switch (option._type) {
		...
		case 'link':
			...
			setTimeout(function () {
				...
				if (mm.is.mobile('ios') || /firefox/i.test(navigator.userAgent)) mm.loading.hide();
...
mm.dropdown = (function () {
	...
	var base = {
		...
		toggle: function (__$ui, __is, __time) {
			...
			else if (mm._isFrame) mm.frameResize();
...
mm.form.multiple = (function () {
	...
	(function () {
		...
		mm.delegate.on(document, '.mm_form-multiple', 'dragover dragenter dragleave dragend drop', function (__e) {
			...
			switch (__e.type) {
				case 'drop':
					if (__e.dataTransfer.files.length === 0) return;
...
mm.popup = (function () {
	...
	return {
		...
		open: function (__url, __option) {
			...
			var resizes = { _width: option.specs._width, _height: option.specs._height };
...
mm.datepicker = (function () {
	...
	var base = {
		get _dataName() { return 'data-datepicker'; },
		checkFormat: function (__date, __data) {
			...
			var dateSeparators = __date.match(/\D+/g);
			var formatSplits = _format.match(/[YMD]+/g);
			var formatSeparators = _format.match(/[^YMD]+/g);
		...
		changeFormat: function (__date, __data) {
			...
			var _format = (function (__format) {
				...
				if (__data && __data.config._isMonth === true && __format.includes('D')) {
					...
					var formatSeparators = __format.match(/[^YMD]+/g);
					var _dayIndex = formatSplits.findIndex(function (__bit) { return __bit.includes('D'); });

					formatSplits.splice(_dayIndex, 1);

					if (!__format.split('').pop().match(/[YMD]+/g)) formatSeparators.splice(_dayIndex, 1);
		...
		update: function (__elements) {
			...
			_.forEach($elements, function (__$el) {
				...
				data.__.onSelect = function (__e) {
					...
					var _pickDate = $item.dataset.datepickerDate;
...
mm.ui = (function () {
	...
	var base = {
		updates: ['tab', 'dropdown', 'stepper', 'carousel', 'slider', 'form', 'preload', 'lazyload', 'datepicker'],
	};
